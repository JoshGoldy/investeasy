...
function buildFinBotPrompt(modeId) {
  ...
  const accuracySystem = system.replace(
    'End every response',
    'Use any supplied FinBot Evidence Pack as the primary source for current prices, holdings, dates, article details, and app-specific facts. Do not invent current market data, consensus estimates, ratios, targets, or dates when they are not supplied; state that the data is unavailable or label assumptions clearly as estimates. Include a short "Data used" or "Data gaps" note when the evidence pack materially affects the answer. End every response'
  );
  let user;
  switch (modeId) {
    case 'screener':
...
8. Overall signal: Strong Buy / Buy / Neutral / Sell / Strong Sell`;
  ...
  return { system: accuracySystem, user };
}

function compactFinBotObject(obj) {
...
function finbotTickerFromText(value) {
  const text = String(value || '').toUpperCase();
  const match = text.match(/\b[A-Z]{1,6}(?:\.[A-Z])?\b/);
  return match ? match[0] : '';
}

function finbotModeTickers(modeId) {
  const tickers = new Set();
  const add = value => {
    const ticker = finbotTickerFromText(value);
    if (ticker) tickers.add(ticker);
  };
  if (modeId === 'dcf') add(finbotForm.ticker);
  if (modeId === 'technical') add(finbotForm.techTicker);
  if (modeId === 'earnings') add(finbotForm.company);
  if (modeId === 'risk' || modeId === 'builder') {
    dbPortfolio.slice(0, 12).forEach(h => add(h.ticker));
  }
  return [...tickers].slice(0, 12);
}

function finbotMarketSnapshot(ticker, liveQuote = null) {
  const upper = String(ticker || '').toUpperCase();
  const market = MARKETS.find(m => (
    m.ticker === upper ||
    marketDataTicker(m) === upper ||
    marketDataTicker(m) === ticker
  ));
  const quote = liveQuote || null;
  return compactFinBotObject({
    ticker: upper,
    dataTicker: market ? marketDataTicker(market) : upper,
    name: market?.name,
    sector: market?.sector,
    exchange: market?.exchange || 'US',
    currency: marketCurrency(market),
    price: quote?.price ?? market?.val,
    dayChangePct: quote?.chg ?? market?.chg,
    week52High: quote?.hi52 ?? market?.hi52,
    week52Low: quote?.lo52 ?? market?.lo52,
    marketCap: market?.mktcap,
    pe: market?.pe,
    volume: market?.vol,
    source: quote ? 'live market-data function' : (market ? 'app market snapshot' : 'user input only'),
  });
}

function finbotRelevantNews(tickers) {
  const terms = tickers.map(t => String(t || '').toUpperCase()).filter(Boolean);
  if (!terms.length) return [];
  return (liveNews.length ? liveNews : STATIC_FALLBACK_NEWS)
    .filter(article => {
      const text = `${article.title || ''} ${article.description || ''} ${article.ticker || ''}`.toUpperCase();
      return terms.some(term => text.includes(term));
    })
    .slice(0, 6)
    .map(article => compactFinBotObject({
      title: article.title,
      source: article.source || article.publisher,
      category: article.cat,
      publishedAt: article.pubTime ? new Date(article.pubTime).toISOString() : article.published_at,
      url: article.url,
      summary: article.description,
    }));
}

function finbotPortfolioEvidence() {
  if (!dbPortfolio.length) return null;
  const holdings = dbPortfolio.map(h => {
    const market = MARKETS.find(m => m.ticker === h.ticker) || RAW_MARKETS.find(m => m.ticker === h.ticker);
    const currency = market?.currency || h.currency || 'USD';
    const currentNative = market ? Number(market.val || 0) : Number(h.avg_cost || 0);
    const valueUsd = Number(h.shares || 0) * nativeToUsd(currentNative, currency);
    const costUsd = Number(h.shares || 0) * nativeToUsd(Number(h.avg_cost || 0), currency);
    return compactFinBotObject({
      ticker: h.ticker,
      name: h.name || market?.name,
      shares: Number(h.shares || 0),
      averageCostNative: Number(h.avg_cost || 0),
      currentPriceNative: currentNative,
      currency,
      sector: market?.sector,
      valueUsd: Number(valueUsd.toFixed(2)),
      unrealizedPnlUsd: Number((valueUsd - costUsd).toFixed(2)),
    });
  });
  const totalValueUsd = holdings.reduce((sum, h) => sum + Number(h.valueUsd || 0), 0);
  return {
    totalValueUsd: Number(totalValueUsd.toFixed(2)),
    displayCurrency: loadSettings().currency,
    holdings: holdings.slice(0, 20),
  };
}

async function buildFinBotEvidencePack(modeId) {
  const tickers = finbotModeTickers(modeId);
  const evidence = {
    generatedAt: new Date().toISOString(),
    mode: modeId,
    appContext: compactFinBotObject({
      displayCurrency: loadSettings().currency,
      balancesHidden: Boolean(loadSettings().hideBalances),
      userTier: currentUser?.tier,
    }),
    userInputs: compactFinBotObject({
      ticker: finbotForm.ticker,
      techTicker: finbotForm.techTicker,
      company: finbotForm.company,
      earningsDate: finbotForm.earnDate,
      riskTolerance: finbotForm.risk || finbotForm.bRisk,
      amount: finbotForm.amount,
      horizon: finbotForm.horizon,
      sectors: finbotForm.sectors,
      goal: finbotForm.goal,
      accountType: finbotForm.acct,
    }),
    tickers,
  };
  ...
  if (tickers.length) {
    try {
      const quoteData = await fetchMarketData('quotes', { tickers: tickers.join(',') });
      liveQuotes = quoteData?.quotes || {};
      if (modeId === 'technical' && finbotForm.techTicker) {
        const key = finbotTickerFromText(finbotForm.techTicker);
        const q = liveQuotes[key] || liveQuotes[finbotForm.techTicker];
      ...
      evidence.liveQuoteError = describeAppError(error, 'Live quotes unavailable; app snapshot used instead.');
      if (modeId === 'technical') finbotForm.techLivePrice = null;
    }
  }

  evidence.marketSnapshots = tickers.map(ticker => finbotMarketSnapshot(ticker, liveQuotes[ticker]));
  evidence.relevantNews = finbotRelevantNews(tickers);
  if (modeId === 'risk' || modeId === 'builder') evidence.portfolio = finbotPortfolioEvidence();
  return evidence;
}
...
async function runFinBot(modeId) {
  ...
  try {
    const evidence = await buildFinBotEvidencePack(modeId);
    const data = await invokeFinBotFunction({ ...buildFinBotPrompt(modeId), evidence, mode: modeId, request_type: 'finbot' });
    if (data.error) {
